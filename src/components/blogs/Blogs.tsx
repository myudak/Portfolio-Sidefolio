            ...
            <AnimatePresence mode="wait">
              {hovered === blog.slug && (
                <motion.div
                  ...
                  className="absolute z-0 pointer-events-none dark:bg-gray-950 bg-gray-50 inset-0 h-full w-full   rounded-md "
                />
              )}
            </AnimatePresence>
              ...
              <div className="flex flex-col col-span-3">
                ...
                <div className="flex space-x-2 flex-wrap mt-4">
                  {blog.tags?.map((tag, index) => (
                    <span
                      key={`tag-${blog.slug}`}
                      className="text-xs px-1 py-0.5 text-secondary border dark:bg-black dark:border-neutral-900 border-neutral-200 bg-white rounded-md"
                    >
                      {tag}
                    </span>
                  ))}
